...
目的是打印遍历的顺序，前中后则是遍历规则，用来简化程序代码。按照规往下遍历下一个节点，最终都会走到叶子节点（比如中序，先遍历到左分支的叶子，期间将所有的左节点已经入栈，那么出栈的时候自然就是先左后根，到右的时候还需要继续压入所有左节点）；而走完左根右三个顺序之后就需要回溯到上一节点，再按照规则继续。回溯到根节点，重复规则直至所有子分支都遍历完成。
