...
⑵遍历该结点的左子树（L），
⑶遍历该结点的右子树（R）。
也就是说三种操作有六种执行次序组合：NLR、LNR、LRN、NRL、RNL、RLN。但是前三种次序与后三种次序对称，故只讨论先左后右的前三种次序。因此我们做出简化，分为前中后，并规定左右节点顺序是先左后右，从而去掉了对称的三种。

既不管是前序、中序还是后序遍历，**变的只是根节点的位置**， 左右节点的顺序永远是先左后右。 比如前序遍历就是根在前面，即根左右（遍历由根往左既前）；中序就是根在中间，即左根右（遍历由左往根既中）；后序就是根在后面，即左右根（遍历由左往右既后）。


下面我们依次讲解：

## 前序遍历
NLR：前序遍历(Preorder Traversal 亦称（先序遍历或先根遍历））
...
## 中序遍历
LNR：中序遍历(Inorder Traversal 亦称（中序遍历或中根遍历）)
...
中序遍历的顺序是 `左-根-右`，根节点不是先输出，这就有一点点复杂了。利用栈的算法如下：
1. 根节点入栈
...
## 后序遍历
LRN：后序遍历(Postorder Traversal 亦称（后序遍历或后根遍历）)
...
二叉树深度优先遍历
前序遍历： 0144.二叉树的前序遍历
后序遍历： [0145.二叉树的后序遍历](../problems/145.binary-tree-postorder-traversal.md)
...
先帮大家明确一下二叉树的遍历规则：
![前中后序遍历](https://pic.leetcode-cn.com/0005d6f797d3281bbe2be08effd0f8fa991dc8126aef754929af34edf650626a-image.png)
