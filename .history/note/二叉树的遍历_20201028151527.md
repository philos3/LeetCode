...
思路是：
按照规则遍历是简化程序代码，但是我们最终需要的输出因为层次问题，和遍历的顺序是不一致的，所以需要借助其他结构进行处理,这里利用栈的先入后出特性，用栈处理，则思路也对应的划分为哪时候入栈，哪时候出栈（出栈为我们需要的节点），递归则是重复入栈和出栈的执行。
